      ...
      <div class="container-fluid">
        ...
        <div class="row">
         <div class="form-group input-group" align="right">
          <input name="consulta" id="txt_consulta" placeholder="Filtrar alunos" type="text" class="form-control">
        </div>
        <div class="col-md-12" >
          ...
          <tbody>
            ...
            $result = $conn->prepare("SELECT * FROM aluno INNER JOIN turma ON turma.id_turma = aluno.id_turma_fk 
              ORDER BY serie_turma, nome_aluno LIMIT 45");


            $i = 1;
            $itempg = 45;
            $query_pg = $conn->query("SELECT * FROM aluno INNER JOIN turma ON turma.id_turma = aluno.id_turma_fk 
              ORDER BY serie_turma, nome_aluno");
            $count = $query_pg->rowCount();
            $calculate = ceil($count/$itempg);

            if(isset($_GET['page']) == $i){
             $url = $_GET['page'];
             $mody = $url*45 - 45;

             $result = $conn->query("SELECT * FROM aluno INNER JOIN turma ON turma.id_turma = aluno.id_turma_fk 
              ORDER BY serie_turma, nome_aluno LIMIT 45 OFFSET $mody");
           }
           ...
           for($i=0; $row = $result->fetch(); $i++){
             ...
             ?>
             <tr>
               <td style="text-align:center; word-break:break-all; width:300px;"> <?php echo $row ['id_aluno']; ?></td>
               <td style="text-align:center; word-break:break-all; width:200px;"> <?php echo $row ['nome_aluno']; ?></td>
               ...
               <td style="text-align:center; width:350px;">
                <a href="acoes/editar_aluno.php<?php echo '?id='.$id; ?>" class="btn btn-info">Editar</a>
                 ...
                 <div class="modal-dialog">
                  ...
                  <div class="modal-content">
                    <div class="modal-header">
                      ...
                      <a href="listar_aluno.php"class="close">&times;</a>
                    </div>
                    <div class="modal-body">
                      <p><div style="font-size:larger;" class="alert alert-danger">Tem certeza de que quer Excluir?<br>  <b style="color:red;"><?php echo $row['nome_aluno']." | ".$row ['serie_turma']."".$row['curso']; ?></b></p>
                      </div>
                      <div class="modal-footer">
                        <a href="listar_aluno.php" class="btn btn-success">Não</a>
                        <a href="acoes/delete_aluno.php<?php echo '?matricula_fk='.$id; ?>" class="btn btn-danger">Sim</a>
                      </div>
                    </div>
                  </div>
                </center>
        ...
        $i = 1;
        while($i <= $calculate){
          echo "<a href='?page=$i' class='btn btn-success' >$i</a>";
          echo " ";
          $i++;
        }
        ?>
