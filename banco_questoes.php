        ...
        <div class="col-md-12">
            <div class="table-responsive"> 
                <table class="table table-border table-striped" style="width: 100%;">
                    <?php 
                    $result = $conn->prepare("SELECT * FROM questoes INNER JOIN assunto ON assunto.id_assunto = questoes.id_assunto_fk INNER JOIN disciplina ON disciplina.id_disciplina = questoes.id_disciplina_fk");
                    $result->execute();
                    for($i=0; $row = $result->fetch(); $i++){
                        echo "<tr>";
                        echo "<th>ID</th>";
